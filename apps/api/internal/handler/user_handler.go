...
// @Success      200  {array}   models.UserXPHistory
// @Failure      401  {object}  map[string]string
// @Failure      500  {object}  map[string]string
// @Router       /me/xp [get]
func (h *Handler) GetMyXPHistory(w http.ResponseWriter, r *http.Request) {
	userID, ok := middleware.UserIDFromContext(r.Context())
	if !ok {
		httpx.JSONError(w, http.StatusUnauthorized, "Unauthorized")
		return
	...
	history, err := h.UserRepo.GetXPHistory(userID, limit, offset)
	if err != nil {
		httpx.JSONError(w, http.StatusInternalServerError, "Failed to get XP history")
		return
	}
