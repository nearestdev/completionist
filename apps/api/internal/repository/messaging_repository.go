...
func (r *MessagingRepository) SendDirectMessage(msg *models.DirectMessage) (*models.DirectMessage, error) {
	...
	err := r.DB.QueryRowx(`
		INSERT INTO direct_messages (sender_id, receiver_id, content, reply_to_id, is_pinned)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, sender_id, receiver_id, content, is_read, reply_to_id, is_pinned, created_at
	`, msg.SenderID, msg.ReceiverID, msg.Content, msg.ReplyToID, msg.IsPinned).StructScan(&result)
	if err != nil {
		return nil, err
	}
	return &result, nil
...
func (r *MessagingRepository) GetConversation(userID, otherUserID int64, limit int, beforeID int64) ([]models.DirectMessageResponse, error) {
	...
	if len(messages) > 0 {
		...
		query, args, err := sqlx.In(`
			SELECT 
				dmr.id, dmr.message_id, dmr.user_id, u.username, dmr.reaction, dmr.created_at
			FROM direct_message_reactions dmr
			JOIN users u ON dmr.user_id = u.id
			WHERE dmr.message_id IN (?)
		`, msgIDs)
		if err == nil {
			...
			if err == nil {
				...
				for i := range messages {
					if reacts, ok := reactionsMap[messages[i].ID]; ok {
...
func (r *MessagingRepository) SendRoomMessage(msg *models.RoomMessage) (*models.RoomMessage, error) {
	...
	err := r.DB.QueryRowx(`
		INSERT INTO room_messages (room_id, user_id, content, reply_to_id, is_pinned)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, room_id, user_id, content, reply_to_id, is_pinned, created_at
	`, msg.RoomID, msg.UserID, msg.Content, msg.ReplyToID, msg.IsPinned).StructScan(&result)
	if err != nil {
		return nil, err
	}
	return &result, nil
...
func (r *MessagingRepository) GetRoomMessages(roomID int64, limit int) ([]models.RoomMessageResponse, error) {
    ...
    if len(messages) > 0 {
		...
		query, args, err := sqlx.In(`
			SELECT 
				rmr.id, rmr.message_id, rmr.user_id, u.username, rmr.reaction, rmr.created_at
			FROM room_message_reactions rmr
			JOIN users u ON rmr.user_id = u.id
			WHERE rmr.message_id IN (?)
		`, msgIDs)
		if err == nil {
			query = r.DB.Rebind(query)
...
func (r *MessagingRepository) ReactToRoomMessage(userID, messageID int64, reaction string) error {
	_, err := r.DB.Exec(`
		INSERT INTO room_message_reactions (message_id, user_id, reaction)
		VALUES ($1, $2, $3)
		ON CONFLICT (message_id, user_id, reaction) DO NOTHING
	`, messageID, userID, reaction)
	return err
...
func (r *MessagingRepository) RemoveDirectMessageReaction(userID, messageID int64, reaction string) error {
	_, err := r.DB.Exec(`
...
func (r *MessagingRepository) RemoveRoomMessageReaction(userID, messageID int64, reaction string) error {
	_, err := r.DB.Exec(`
...
func (r *MessagingRepository) GetPinnedRoomMessages(roomID int64) ([]models.RoomMessageResponse, error) {
	...
	err := r.DB.Select(&messages, `
		...
		FROM room_messages rm
		JOIN users u ON rm.user_id = u.id
		WHERE rm.room_id = $1 AND rm.is_pinned = true
		ORDER BY rm.created_at DESC
	`, roomID)
	if err != nil {
		return nil, err
