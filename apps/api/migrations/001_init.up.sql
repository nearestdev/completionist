...
CREATE TABLE direct_messages (
  ...
  reply_to_id BIGINT REFERENCES direct_messages(id) ON DELETE SET NULL,
...
CREATE TABLE room_messages (
  ...
  user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  content TEXT NOT NULL,
  reply_to_id BIGINT REFERENCES room_messages(id) ON DELETE SET NULL,
  is_pinned BOOLEAN NOT NULL DEFAULT FALSE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX idx_dm_reactions_message ON direct_message_reactions(message_id);
...
CREATE INDEX idx_room_reactions_message ON room_message_reactions(message_id);