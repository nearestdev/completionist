...
export default function MusicRoom({ room }: MusicRoomProps) {
  ...
  const handleLoadMedia = async () => {
    ...
    try {
      await roomService.updateRoomState(room.id, {
        ...
        currentMediaTitle: extractTitleFromUrl(mediaUrl),
        currentPositionMs: 0,
        isPlaying: true,
      });
      setMediaUrl('');
  ...
  return (
    <div className="flex h-full gap-4">
          ...
          {youtubeId ? (
            ...
            <div className="flex-1 flex items-center justify-center text-muted">
              <div className="text-center">
                ...
                <p className="text-sm text-muted/70">Share a YouTube link to start listening together</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="w-96 flex flex-col border-l border-border">
        <div className="p-4 border-b border-border flex items-center gap-2 text-muted">
          <UsersIcon size={20} />
          <span className="text-sm">{members} listening</span>
        </div>
