...
import { BookOpenIcon, MagnifyingGlassIcon, SpinnerGapIcon, ArrowLeftIcon } from "@phosphor-icons/react/dist/ssr";

function MangaSearchPageContent() {
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    setError(null);
    try {
      const response = await jikanService.searchManga(searchQuery);
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-7xl animate-in fade-in duration-500">
      <div className="text-center mb-12 space-y-4">
        <div className="inline-flex items-center justify-center p-4 bg-gradient-to-br from-slate-400 to-slate-500 rounded-2xl mb-4 shadow-lg">
          <BookOpenIcon size={40} weight="fill" className="text-white" />
        </div>
        <h1 className="text-4xl md:text-5xl font-heading font-bold text-foreground tracking-tight">
          Search Manga
        </h1>
        <p className="text-xl text-muted max-w-2xl mx-auto">
          Graphic novels and serialized comics
        </p>
      </div>
