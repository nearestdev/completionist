...
export default function Widgets() {
  ...
  return (
    <aside 
      className="hidden xl:flex flex-col w-[320px] p-6 space-y-6 h-screen sticky top-0 overflow-y-auto border-l border-border bg-background"
      style={{
        background: 'linear-gradient(to bottom, var(--background), rgba(99, 102, 241, 0.02))'
      }}
    >
        ...
        {loading ? (
          ...
          <ul className="space-y-4">
            {suggestions.map((user) => (
              <li key={user.userId} className="flex items-center justify-between gap-3 group">
                <div className="flex items-center gap-3 overflow-hidden">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center text-primary font-bold text-xs flex-shrink-0">
